...
function checkEnv() {
  ...
  if (!anthropic) {
    warn(
      "ANTHROPIC_API_KEY is not set — AI moment summaries will be disabled",
      "Add ANTHROPIC_API_KEY to .env for natural-language highlight summaries. " +
        "Detection, clipping, and annotation still work without it.",
    );
  }
}
...
// The CV pipeline shells out to ffmpeg/ffprobe to probe, cut clips, and
// encode the annotated video. These are system binaries, not pip packages.
function checkFfmpeg() {
  for (const bin of ["ffmpeg", "ffprobe"]) {
    if (!tryExec(`${bin} -version`)) {
      fail(
        `${bin} is not on PATH (required by the video pipeline)`,
        "Install ffmpeg: macOS `brew install ffmpeg`, Debian/Ubuntu " +
          "`sudo apt install ffmpeg`, or https://ffmpeg.org/download.html",
      );
    }
  }
}
